...
def get_mac_address(conn, vm_name):
...
def get_dnsmasq_dhcp_lease(conn, vm_name):
    ...
    """
    # We're looking for domain name instead of hostname, therefore the
    # 'vagrant_' prefix has to be added.
    vm_name = 'vagrant_' + vm_name
    mac_address = get_mac_address(conn, vm_name)

    with open('/var/lib/libvirt/dnsmasq/default.leases') as leases_file:
