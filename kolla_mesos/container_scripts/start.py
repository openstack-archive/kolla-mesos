...
DEPLOYMENT_ID = os.environ.get('DEPLOYMENT_ID', 'undefined')
...
def main():
    ...
    with zk_connection(ZK_HOSTS) as zk:
        ...
        if register_group:
            ...
            # Step 1 - render general config
            service_conf = render_config(zk, service_conf_raw)
            LOG.info('Rendered service config: %s' % service_conf)
            # Step 2 - render and put config files
