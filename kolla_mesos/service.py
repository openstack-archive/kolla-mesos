...
class Runner(object):
    ...
    def load_from_zk(cls, zk, service_name):
        ...
        try:
            ...
            raise KeyError('node not found %s' % dest_node)
        return cls(conf)
...
def list_services():
    return _list_chronos() + _list_marathon()
...
def scale(service_name, instances, force=False):
    if '/' not in service_name:
        raise TypeError("can't scale a chronos task dude")
