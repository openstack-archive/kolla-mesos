...
class Runner(object):
    ...
    def snapshot(self, zk, output_dir, variables):
        ...
        The files within the directory should be:
        variables.yml, <service>.yml.j2, <service>.{marathon|chronos}, files/*
        """
        ...
        # variables
        with open(os.path.join(output_dir, 'variables.yml')) as vfp:
            vfp.write(yaml.dump(variables))

        # service definition and files
        with open(os.path.join(proj_dir, '%s.yml.j2' % service_name)) as sfp:
        ...
        if zk.exists(files_node):
            file_utils.mkdir_p(os.path.join(proj_dir, 'files'))
            try:
                files = zk.get_children(files_node)
            except exceptions.NoNodeError:
                files = []
            for fn in files:
                with open(os.path.join(proj_dir, 'files', fn)) as fp:
...
def _load_variables_from_zk(zk):
    ...
    for var in var_names:
        value, stat = zk.get(os.path.join(path, var))
