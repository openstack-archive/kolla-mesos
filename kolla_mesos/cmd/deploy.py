...
class Runner(object):
    ...
    def generate_deployment_files(self, kolla_config, jinja_vars, temp_dir):
        ...
        LOG.debug(json.dumps(app_def, indent=2))
...
class KollaWorker(object):
    ...
    def process_project_config(self, zk, proj, conf_path, jinja_vars,
                               kolla_config):
        LOG.warn('old school config not supported %s' % conf_path)
    ...
    def process_service_config(self, zk, proj, conf_path,
        ...
        if 'service' in conf:
            LOG.info('Found MarathonApp: %s', conf_path)
            runner = MarathonApp(conf)
        else:
            LOG.info('Found ChronosTask: %s', conf_path)
    ...
    def write_config_to_zookeeper(self, zk):
        ...
        common_cfg = self._write_common_config_to_zookeeper(zk, jinja_vars)
