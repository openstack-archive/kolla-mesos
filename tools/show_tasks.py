...
CONFIG_DIR = '/usr/share/kolla-mesos/config/'
...
def print_status(status):
    ...
    rows = [('Task', 'Register (/kolla/variables/)', 'Reg Sts',
...
def pretty_print(table):
    ...
    Table is a list of rows containing column data. The first row is a header.
    ...
    """
    widths = [len(max(columns, key=len)) for columns in zip(*table)]

    sep_printed = False
    for row in table:
        rowout = '| '
        for width, col_data in zip(widths, row):
            rowout += format(col_data, '%ds' % width) + ' | '
        print(rowout)
        if not sep_printed:
            # print a separator between header and row data
            rowout = '+'
            for width in widths:
                rowout += '-' * width + '--+'
                sep_printed = True
