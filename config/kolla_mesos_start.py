...
class Command(object):
        ...
        if self.run_once:
            def _init_done():
                if not self.check_path:
                    raise ValueError('"run_once" True but no "register"')
                return zk.retry(zk.exists, self.check_path)

            if _init_done():
                print("Path '%s' exists: init was already done" %
                      self.check_path)
                print("Skipping init command")
            else:
                print("Path '%s' not exists: run init" % self.check_path)
                zk.retry(zk.ensure_path, self.init_path)
                lock = zk.Lock(self.init_path)
                print("Acquiring lock '%s'" % self.init_path)
                with lock:
                    if not _init_done():
                        print("Path '%s' not exists: run init" %
                              self.check_path)
                        self._run_command()
                print("Releasing lock '%s'" % self.init_path)
        else:
            self._run_command()
        print('*' * 80)
    ...
    def _run_command(self):
        ...
        if child_p.returncode != 0:
            print("Command %s non-zero code %s" % (
                self, child_p.returncode))
            sys.exit(1)
        if self.check_path:
            print("Marking init '%s' as done" % self.name)
            self.zk.retry(self.zk.ensure_path, self.check_path)
            print("Init '%s' marked as done" % self.name)
